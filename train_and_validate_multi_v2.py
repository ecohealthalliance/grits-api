...
class MultiLevelTester():
    ...
    def main(self):
        ...
        # Do articles labeled with a child disease always get labeled with the
        # parent disease as well?
        print "Checking if parent labels are always included..."
        for labels in y_training:
            for label in labels:
                if label in self.disease_to_parent:
                    if not self.disease_to_parent[label] in labels:
                        print "Example contained label", label, "but not parent label", self.disease_to_parent[label], "in labels:", labels
        ...
        # Re-map the training labels to put them all into the parent disease space,
        # when possible
        y_training_parentalized = map(
            lambda labels: list(set(map(
                (lambda label: self.disease_to_parent[label]
                          if label in self.disease_to_parent
                          else label),labels))), y_training)

        print "Size of training set:", len(X_training)
        ...
        print "Getting and fitting top_classifier..."
        # top_classifier = self.get_fit_and_dump_one_v_rest_classifier(
        #     X_training, y_training_parentalized)
        top_classifier = self.load_classifier()
        print "top_classifier built or loaded."

        # Build an individual classifier for each parent disease that has children
        print "Building child classifiers..."
        child_classifiers = {}
        for parent, children in self.parent_to_diseases.iteritems():
            print "building parent:", parent, "children:", children, "classifier"
            X_child = []
            y_child = []
            for X, y in zip(X_training, y_training):
                if (len(set(children).intersection(set(y))) > 0):
                    print "y contains:", y
                    X_child.append(X)
                    y_child.append(filter(lambda label: (label in children), y))
            if len(X_child):
                print "Got data for children:", children, "parent:", parent
                print "labels: ", y_child
                child_classifier = self.get_fit_and_dump_one_v_rest_classifier(
                    X_child,
                    y_child, 
                    pickle_filename="one_vs_rest_classifier_" + parent + ".pickle")
                child_classifiers[parent] = child_classifier
            else:
                print "No data found for parent:", parent, "children:", children, "classifier"


        # classifier = self.load_classifier()
        print "Child classifiers loaded."

        print "Getting training and validation predictions"
        # top_training_predictions = self.get_classifier_predictions(top_classifier, X_training)
        top_validation_predictions = self.get_classifier_predictions(top_classifier, X_validation)
        print "Got top predictions"

        # training_predictions = []
        # for predictions, data in zip(top_training_predictions, X_training):
        #     child_predictions = map(lambda prediction: 
        #         self.perform_child_classification(prediction, data, child_classifiers),
        #         predictions)
        #     training_predictions.append(predictions + child_predictions)

        validation_predictions = []
        for predictions, data in zip(top_validation_predictions, X_validation):
            child_predictions = tuple(flatten(map(lambda prediction: 
                self.perform_child_classification(prediction, data, child_classifiers),
                predictions)))
            print "predictions:", predictions
            print "child_predictions:", child_predictions
            validation_predictions.append(predictions + child_predictions)
        ...
        print "Validation set micro:\nprecision: %s recall: %s f-score: %s" % \
            sklearn.metrics.precision_recall_fscore_support(y_validation, validation_predictions, average='micro')[0:3]

        # self.get_metrics_with_multilevel_awareness(validation_predictions, y_validation)

    @timed
    def perform_child_classification(self, prediction, data, child_classifiers):
        if not (prediction in child_classifiers):
            return tuple()
        else:
            child_classifier = child_classifiers[prediction]
            return tuple(
                [child_classifier.classes_[i]
                 for i, p in self.best_guess(child_classifier, data)])
    ...
    def best_guess(self, classifier, X):
        try :
            ...
            p_max = max(probs)
            guesses = [(i,p) for i,p in enumerate(probs) if p >= p_max * self.cutoff_ratio]
            return guesses
        except AttributeError :
            prediction = classifier.predict(X)[0]
            print "prediction: ", prediction[0]
            print "type(prediction): ", type(prediction)
            return [(classifier.classes_.tolist().index(prediction[0]), 1.0)]
    ...
    def get_metrics_with_multilevel_awareness(self, validation_predictions, y_validation):
        ...
        # how many labels are children / parents / unlinked ?
        child_label_count = 0
        parent_label_count = 0
        unlinked_label_count = 0

        for predictions, labels in zip(validation_predictions, y_validation):

            for label in labels:
                if label in self.parent_to_diseases:
                    parent_label_count += 1
                elif label in self.disease_to_parent:
                    child_label_count += 1
                else:
                    unlinked_label_count += 1
        ...
        print "parent_label_count:", parent_label_count
        print "child_label_count:", child_label_count
        print "unlinked_label_count:", unlinked_label_count
